...
/*
    ...
    const onFinishDR = (record) => {
      let now = moment().format('YYYY-MM-DD');
      axios.post("api/dailyReport/add", { reqDate:now,
        animalId: animalId, createdAt: now, userId: localStorage.getItem("userId"), description:record.description, location: record.location, status: record.status})
     .then(res=>{
       console.log(res.data.message);  
